...
- If a `project.md` exists, follow its documented conventions. Project knowledge lives either in `.aria/` or, in personal mode, in `~/.claude/aria/projects/<slug>/` where the slug comes from `--git-common-dir` so every worktree of a repository shares one location — see [KNOWLEDGE-LOCATION.md](../setup/KNOWLEDGE-LOCATION.md). **Never create either directory.**
...
5. If project knowledge exists in either mode: offer aria:learn to capture session learnings
